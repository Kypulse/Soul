...
public class AutoShulker extends Module {
    ...
    public AutoShulker() {
        super(SoulAddon.CATEGORY, "AutoShulkerOrder", "Buys shulker boxes and then completes orders on DonutSMP.");
    }
        ...
        switch (stage) {
            ...
            case SHOP_END -> {
                if (mc.currentScreen instanceof GenericContainerScreen screen) {
                    ...
                    if (now - stageStart > 5000) {
                        ...
                        ChatUtils.warning("[AutoShulker] Timeout on SHOP_END, restarting cycle.");
                        stage = Stage.SHOP;
                        stageStart = now;
                    }
                }
            ...
            case SHOP_SHULKER -> {
                ...
                if (mc.currentScreen instanceof GenericContainerScreen screen) {
                    ...
                    if (now - stageStart > 5000) {
                        mc.player.closeHandledScreen();
                        ChatUtils.warning("[AutoShulker] Timeout on SHOP_SHULKER, restarting cycle.");
                        stage = Stage.SHOP;
                        stageStart = now;
                    }
                }
            ...
            case SHOP_CONFIRM -> {
                if (mc.currentScreen instanceof GenericContainerScreen screen) {
                    ...
                    if (now - stageStart > 5000) {
                        mc.player.closeHandledScreen();
                        ChatUtils.warning("[AutoShulker] Timeout on SHOP_CONFIRM, restarting cycle.");
                        stage = Stage.SHOP;
                        stageStart = now;
                    }
                }
            ...
            case SHOP_CHECK_FULL -> {
                ...
                if (now - stageStart > 5000) {
                    mc.player.closeHandledScreen();
                    ChatUtils.warning("[AutoShulker] Timeout on SHOP_CHECK_FULL, restarting cycle.");
                    stage = Stage.SHOP;
                    stageStart = now;
                }
            }
            case SHOP_EXIT -> {
                ...
                if (now - stageStart > 5000) {
                    mc.player.closeHandledScreen();
                    ChatUtils.warning("[AutoShulker] Timeout on SHOP_EXIT, restarting cycle.");
                    stage = Stage.SHOP;
                    stageStart = now;
                }
            }
            ...
            case ORDERS -> {
                if (mc.currentScreen instanceof GenericContainerScreen screen) {
                    ...
                    if (now - stageStart > 5000) {
                        mc.player.closeHandledScreen();
                        ChatUtils.warning("[AutoShulker] Timeout on ORDERS, restarting cycle.");
                        stage = Stage.SHOP;
                        stageStart = now;
                    }
                }
            ...
            case ORDERS_CONFIRM -> {
                if (mc.currentScreen instanceof GenericContainerScreen screen) {
                    ...
                    if (now - stageStart > 5000) {
                        mc.player.closeHandledScreen();
                        ChatUtils.warning("[AutoShulker] Timeout on ORDERS_CONFIRM, restarting cycle.");
                        stage = Stage.SHOP;
                        stageStart = now;
                    }
                }
